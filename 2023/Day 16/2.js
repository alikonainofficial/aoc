...
const traceBeamReflections = (matrix, row, col, dir) => {

    let rayQueue = [];
    rayQueue.push([row , col , dir]);
...
async function processLineByLine() {
    ...
    let originalMatrix = rayMatrix.map(innerArray => [...innerArray]);
    let energizedTileMatrix = [];
    let total = 0;
    for(let i = 0; i < 4; i++){
        if(i == 0){
            for(let j = 0; j < rayMatrix[0].length; j++){
                let tempMatrix = originalMatrix.map(innerArray => [...innerArray]);
                energizedTileMatrix = traceBeamReflections(tempMatrix, i, j, -1);
                let temp = getEnergizedTiles(energizedTileMatrix);
                if(temp > total){
                    total = temp;
                }
            }
        } else if(i == 1){
            for(let j = 0; j < rayMatrix.length; j++){
                let tempMatrix = originalMatrix.map(innerArray => [...innerArray]);
                energizedTileMatrix = traceBeamReflections(tempMatrix, j, 0, -2);
                let temp = getEnergizedTiles(energizedTileMatrix);
                if(temp > total){
                    total = temp;
                }
            ...
            for(let j = 0; j < rayMatrix.length; j++){
                let tempMatrix = originalMatrix.map(innerArray => [...innerArray]);
                energizedTileMatrix = traceBeamReflections(tempMatrix, j, rayMatrix[0].length - 1, 2);
                let temp = getEnergizedTiles(energizedTileMatrix);
                if(temp > total){
                    total = temp;
                }
            ...
            for(let j = 0; j < rayMatrix[0].length; j++){
                let tempMatrix = originalMatrix.map(innerArray => [...innerArray]);
                energizedTileMatrix = traceBeamReflections(tempMatrix, rayMatrix.length - 1, j, 1);
                let temp = getEnergizedTiles(energizedTileMatrix);
                if(temp > total){
                    total = temp;
                }
